...
export class QuizComponent implements OnInit {
  ...
  get upcomingQuizzes(): ScheduledQuiz[] {
    const now = Date.now();
    return this.scheduledQuizzes.filter((quiz) => quiz.scheduledAt.getTime() >= now);
  }
  ...
  get upcomingRangeStart(): number {
    if (!this.upcomingQuizzes.length) return 0;
  ...
  ngOnInit(): void {
    ...
    if (id) {
      ...
      this.quizService.getQuizzesByCourse(this.courseId).subscribe({
        next: (data) => this.handleLoadedQuizzes(data),
        error: (error) => {
  ...
  // ✅ CORRIGÉ : demande l'ID du cours puis navigue vers AddQuizComponent
  openQuizCreation() {
    const courseIdInput = prompt('Entrez l\'ID du cours pour créer le quiz :');
    ...
    this.router.navigate(['/admin/quizzes/add', courseId]);
  }
  ...
  saveSchedule(quiz: ScheduledQuiz) {
    if (!this.scheduleDraftValue) return;
  ...
  goToUpcomingPage(page: number) {
    if (page < 1 || page > this.totalUpcomingPages || page === this.upcomingPage) return;
